...
async def get_active_journeys(current_user: Dict[str, Any] = Depends(verify_token)) -> Dict[str, Any]:
    ...
    try:
        ...
        if journey_engine is None:
            # Try to sync SmartMoving data first, then fetch from database
            try:
                # Import SmartMoving sync service
                from ..services.smartmoving_sync_service import SmartMovingSyncService
                
                # Sync SmartMoving jobs for today and tomorrow
                async with SmartMovingSyncService() as sync_service:
                    sync_result = await sync_service.sync_today_and_tomorrow_jobs()
                    print(f"SmartMoving sync result: {sync_result}")
                
                # Now fetch the synced journeys from database
                conn = get_db_connection()
                cursor = conn.cursor(cursor_factory=RealDictCursor)
                
                # Get journeys for the user's location
                user_location_id = current_user.get("location_id")
                user_company_id = current_user.get("company_id") or "clm_f55e13de_a5c4_4990_ad02_34bb07187daa"
                
                if user_location_id:
                    # Filter by specific location
                    cursor.execute("""
                        SELECT * FROM "TruckJourney" 
                        WHERE "locationId" = %s AND "clientId" = %s
                        ORDER BY "date" DESC, "createdAt" DESC
                    """, (user_location_id, user_company_id))
                else:
                    # Get all journeys for the company
                    cursor.execute("""
                        SELECT * FROM "TruckJourney" 
                        WHERE "clientId" = %s
                        ORDER BY "date" DESC, "createdAt" DESC
                ...
                return {
                    ...
                    "message": f"Retrieved {len(real_journeys)} SmartMoving journeys from database"
                }
                
            except Exception as e:
                print(f"Error syncing SmartMoving data: {e}")
                # Return empty data if SmartMoving sync fails
                return {
                    "success": True,
                    "data": [],
                    "message": "No SmartMoving journeys found"
                }
        ...
        if not success:
            # If journey engine fails, return empty data instead of test data
            print(f"Journey engine failed: {message}, returning empty data")
            return {
                "success": True,
                "data": [],
...
from apps.api.services.smartmoving_api import SmartMovingAPI
...
async def get_user_journeys(
    ...
    """Get journeys for the current user based on their location and role"""
    try:
        user_id = current_user.get("sub")
        user_location_id = current_user.get("locationId")
        user_role = current_user.get("role", "")
        client_id = current_user.get("clientId")
        
        if not user_location_id or not client_id:
            return {
                "success": False,
                "message": "User location or client information not available",
                "data": []
            }
        
        # For now, return mock journey data until we get real SmartMoving API credentials
        # This simulates real journey data based on the user's location
        mock_journeys = [
            {
                "id": f"journey_mock_001_{user_location_id}",
                "externalId": "mock_001",
                "externalSource": "SmartMoving",
                "truckNumber": f"SM-{user_location_id[:8]}",
                "status": "MORNING_PREP",
                "customerName": "John Smith",
                "moveDate": "2025-08-08T09:00:00Z",
                "estimatedStartTime": "2025-08-08T09:00:00Z",
                "estimatedEndTime": "2025-08-08T17:00:00Z",
                "originAddress": "123 Main Street, Toronto, ON M5J 2N1",
                "destinationAddress": "456 Oak Avenue, Ottawa, ON K1A 0B1",
                "estimatedDuration": 8,
                "estimatedCost": 1250.00,
                "notes": "Full service move - 3 bedroom house with piano",
                "clientId": client_id,
            ...
            {
                "id": f"journey_mock_002_{user_location_id}",
                "externalId": "mock_002",
                "externalSource": "SmartMoving",
                "truckNumber": f"SM-{user_location_id[:8]}",
                "status": "EN_ROUTE",
                "customerName": "Sarah Johnson",
                "moveDate": "2025-08-08T10:30:00Z",
                "estimatedStartTime": "2025-08-08T10:30:00Z",
                "estimatedEndTime": "2025-08-08T15:30:00Z",
                "originAddress": "789 Queen Street, Toronto, ON M5V 2K7",
                "destinationAddress": "321 King Street, Toronto, ON M5V 1J5",
                "estimatedDuration": 5,
                "estimatedCost": 850.00,
                "notes": "Partial move - office relocation",
                "clientId": client_id,
                "locationId": user_location_id,
            ...
            {
                "id": f"journey_mock_003_{user_location_id}",
                "externalId": "mock_003",
                "externalSource": "SmartMoving",
                "truckNumber": f"SM-{user_location_id[:8]}",
                "status": "COMPLETED",
                "customerName": "Mike Wilson",
                "moveDate": "2025-08-07T14:00:00Z",
                "estimatedStartTime": "2025-08-07T14:00:00Z",
                "estimatedEndTime": "2025-08-07T18:00:00Z",
                "originAddress": "555 College Street, Toronto, ON M6G 1A5",
                "destinationAddress": "777 Bloor Street, Toronto, ON M6G 1L3",
                "estimatedDuration": 4,
                "estimatedCost": 650.00,
                "notes": "Studio apartment move - completed successfully",
                "clientId": client_id,
                "locationId": user_location_id,
                "createdAt": "2025-08-07T13:00:00Z",
                "updatedAt": "2025-08-07T18:00:00Z"
            }
        ]
        
        # Filter journeys based on user role (for future implementation)
        if user_role in ["DRIVER", "MOVER"]:
            # Field workers see only their assigned journeys
            filtered_journeys = mock_journeys[:2]  # First 2 journeys
        else:
            # Managers and dispatchers see all journeys
            filtered_journeys = mock_journeys
        
        return {
            "success": True,
            "message": f"Found {len(filtered_journeys)} journeys for user at {user_location_id}",
            "data": filtered_journeys
        }
            
    except Exception as e:
        logger.error(f"Error fetching user journeys: {str(e)}")
        return {
            "success": False,
            "message": f"Failed to fetch user journeys: {str(e)}",
            "data": []
        }
